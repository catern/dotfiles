...
selflessView :: (Eq s, Eq i) => i -> W.StackSet i l a s sd -> W.StackSet i l a s sd
selflessView w ws
...
--   ++
    [((m .|. modm, k), windows $ f i)
        | (i, k) <- zip (XMonad.workspaces conf) [xK_1 .. xK_9]
        , (f, m) <- [(selflessView, 0), (W.shift, shiftMask)]]
    ++
    --
...
--
myLayout = tiled ||| Mirror tiled ||| noBorders (fullscreenFull Full)
  where
